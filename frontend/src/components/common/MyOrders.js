...
import { Grid, TextField, Button, Rating, Alert, Dialog, DialogTitle, DialogActions, DialogContent, DialogContentText, Paper, TableCell, TableHead, TableRow, Table, TableBody, List, IconButton, InputAdornment } from "@mui/material";
...
const Dashboard = (props) => {
	...
	const refresh = () => {
		axios.get("http://localhost:4000/api/order/fetch", { params: { buyer: email } })
			.then((res) => {
				setOrders(res.data);
			})
			.catch((error) => {
	...
	const handleNext = (order) => {
			axios
				.post("http://localhost:4000/api/order/nextState", { order: order, newState: states[4] })
				.then((res) => {
					console.log(res);
					refresh();
				})
	...
	const handleRate = (order, rating) => {
		...
		axios
			.post("http://localhost:4000/api/food/getItem", { shop: order.shop, name: order.item })
			.then((res) => {
				const food = res.data;
				axios
					.post("http://localhost:4000/api/food/setRating", { item: food, newRating: rating })
					.then((res) => {
						console.log(res);
					})
					.catch((error) => {
